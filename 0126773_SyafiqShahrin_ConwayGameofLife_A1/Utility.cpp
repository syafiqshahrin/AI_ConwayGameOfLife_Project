...
void Utility::loadTexturePNG(const char* path, GLuint textureID)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // near filtering. (For when textures needs to scale up on screen)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // far filtering. (For when textures needs to scale down on screen)
